...
static void err_bad_opt(const char* arg, char c, int n) {
    ...
    if (c)
        ...
        if (p)
            err_report("Bad %s `%s' (with arg `%s') (argc %d)", type, arg, p + 1, n);
        else
...
static void err_bad_excl(const CLIF_option* optn, char c, int n) {
    ...
    err_report(
        "%s `%s' (argc %d): Only one of:\n    %s\n"
        "may be specified.",
        (c || !is_keyword(optn)) ? "Option" : "Keyword", ss, n, excl);
}

static CLIF_option* find_long(char* arg, char** arg_p, unsigned int match, unsigned int nomatch) {
    CLIF_option* optn;
    CLIF_option* abbrev = NULL;
    char* abbrev_arg = NULL;
    int abbrev_found = 0;

    for (optn = curr.option_list; optn->short_opt || optn->long_opt; optn++) {
        char* a;
        const char* o;
        unsigned int flags;
        ...
        if (*a == '\0' || (*a == '=' && optn->arg_name && !(flags & _CLIF_STRICT_NOEQUAL))) { /*  looks like end of option...  */
            ...
            if ((flags & CLIF_ABBREV) && (a - arg >= CLIF_MIN_ABBREV)) {
                if (!abbrev_found) {
                    abbrev_found = 1;
                    abbrev = optn;
                    if (*a == '=')
...
static int call_function(CLIF_option* optn, char* opt_arg, char sym) {
    ...
    if (opt_arg && ((optn->flags | curr.parse_flags) & CLIF_SEVERAL)) {
        ...
        while (*opt_arg) {
            t = tmp;
            while (t < endt && *opt_arg && *opt_arg != ' ' && *opt_arg != '\t' && *opt_arg != ',')
                *t++ = *opt_arg++;
...
int CLIF_parse_cmdline(int argc, char* argv[], CLIF_option* option_list, CLIF_argument* argument_list, unsigned int parse_flags) {
    ...
    int dirty_plus = 0;
    int exclusive_cnt = 0;
    int posix = getenv("POSIXLY_CORRECT") != NULL || (parse_flags & CLIF_POSIX);
        ...
        for (argm = argument_list; argm->name; argm++) {
            if (argm->flags & CLIF_STRICT) {
                ...
                if (stage == STRICT_BEG)
                    ...
                    err_report(
                        "Incorrect argument list set in program "
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (*arg != sym) { /*  argument or keyword   */
            ...
            optn = find_long(arg, &opt_arg, 0, _CLIF_STRICT_KEYWORD | _CLIF_STRICT_ONEDASH);
            if (optn)
                goto long_found;
    ...
    long_found:
        ...
        if (optn->arg_name && !opt_arg) {
            ...
            if (++i >= argc || !(flags & CLIF_MAY_NOEQUAL)) { /*  missing opt arg   */
                i--;
    ...
    handle_short:
        ...
        do {
            ...
            if (!optn->short_opt || check_sym(optn, sym) < 0) {
            ...
            if (optn->arg_name) {
                ...
                if (arg[1] == '\0') { /*  a last one   */
                    ...
                    if (++i >= argc || (flags & _CLIF_STRICT_JOIN_ARG)) {
                        i--;
                ...
                else if ((arg == argv[i] || arg[-1] == sym) && (flags & CLIF_MAY_JOIN_ARG)) {
                    opt_arg = ++arg;
                }
                else { /*  inside a group...  */
                    if (!(flags & CLIF_OPTARG) || (flags & CLIF_MAY_JOIN_ARG)) {
                        err_bad_arg(optn, *arg, i);
                        return -1;
                    }
    ...
    if (num_args < strict_beg + strict_end) {
        ...
        if (num_args < strict_beg)
            argm = argument_list + num_args;
        else
            argm = argument_list + ((num_args - strict_beg) + (num_argm - strict_end));

        if (num_args == strict_beg + strict_end - 1)
            err_report("Specify \"%s\" missing argument.", argm->name);
        else
            err_report("Specify \"%s\" and other missing arguments.", argm->name);
        return -1;
    }
    ...
    if (num_args > 0) {
        ...
        for (i = 0, j = 0; i < strict_beg; i++, j++)
            argm_index[i] = j;
        for (i = num_args - strict_end, j = num_argm - strict_end; i < num_args; i++, j++)
            argm_index[i] = j;
        for (i = strict_beg, j = strict_beg; i < num_args - strict_end && j < num_argm - strict_end; i++) {
            argm_index[i] = j;
        ...
        if (i < num_args - strict_end) { /*  there are extra args...  */
            err_report("Extra arg `%s' (position %d, argc %d)", argv[arg_n[i]], i + 1, arg_n[i]);
            return -1;
        }

        if (j < num_argm - strict_end && !(argument_list[j].flags & CLIF_MORE) &&
            /*  ...i.e, there are some missing optional args...  */
            (argument_list[j].flags & CLIF_ACC_PREV)) {
            if (j == 0)
                err_report(
                    "Incorrect argument list set: first arg "
                    "cannot be `accompanied with previous'.");
            else
                err_report(
                    "Arg \"%s\" must be specified because "
                    "\"%s\" `%s' is used.",
                    argument_list[j].name, argument_list[j - 1].name, argv[arg_n[i - 1]]);
            return -1;
        }
        ...
        for (i = 0; i < num_args; i++) {
            ...
            if (argm->function && argm->function(argm, argv[arg_n[i]], i) < 0) {
                err_report(
                    "Cannot handle \"%s\" cmdline arg `%s' "
...
static void box_output(int start, int left, int width, const char* str, const char* arg_name) {
    char *p, *endp, *s;
    int l;
    char buf[1024];
...
void CLIF_print_options(const char* header, const CLIF_option* option_list) {
    const CLIF_option* optn;
    char* excl;
    int excl_cnt = 0;
    ...
    for (optn = option_list; optn->short_opt || optn->long_opt; optn++) {
        ...
        if (optn->short_opt) {
            if (optn->long_opt)
                len = fprintf(stderr, OPT_START_DLM "%s" SHORT_LONG_DLM "%s", show_short(optn), show_long(optn));
            else
                len = fprintf(stderr, OPT_START_DLM "%s", show_short(optn));
        }
        else
        ...
        if (optn->help_string)
            box_output(len, OPT_FIELD_WIDTH, SCREEN_WIDTH - OPT_FIELD_WIDTH, optn->help_string, optn->arg_name);
...
void CLIF_print_arguments(const char* header, const CLIF_argument* argument_list) {
    ...
    for (argm = argument_list; argm->name; argm++) {
        ...
        if (argm->help_string)
            box_output(len, ARG_FIELD_WIDTH, SCREEN_WIDTH - ARG_FIELD_WIDTH, argm->help_string, argm->name);
    ...
    if (option_list) {
        ...
        for (optn = option_list; optn->short_opt || optn->long_opt; optn++) {
            ...
            */
            ...
            if (!optn->short_opt || optn->arg_name || (optn->flags & CLIF_EXCL))
                continue;
